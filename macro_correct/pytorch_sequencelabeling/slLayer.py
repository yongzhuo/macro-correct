...
class PriorMultiLabelSoftMarginLoss(nn.Module):
    def __init__(self, prior=None, num_labels=None, reduction="mean", task_type="SL-GRID", eps=1e-9, tau=1.0):
        """PriorCrossEntropy
        ...
        args:
            ...
            task_type: str, sequence-labeling task type, eg. "SL-GRID", "SL-SPAN", "SL-CRF", "SL-SOFTMAX"
        ...
        self.loss_func = torch.nn.MultiLabelSoftMarginLoss(reduction=reduction)
        if not prior:
            prior = np.array([1/num_labels for _ in range(num_labels)])  # 如果不存在就设置为num
        if type(prior) == list: prior = np.array(prior)
        self.log_prior = torch.tensor(np.log(prior + eps)).unsqueeze(0)
        if task_type.upper() == "SL-GRID":
            self.loss_func = MultiLabelCircleLoss()  # FocalLoss()  # MultiLabelCircleLoss()
            self.log_prior = self.log_prior.unsqueeze(2).unsqueeze(3)
        self.eps = eps
        self.tau = tau
...
class FocalLossCSC(nn.Module):
    def __init__(self, alpha=0.5, gamma=2, eps=1e-6, reduction="mean", activate_type="sigmoid"):
        """FocalLoss
        聚焦损失, 不确定的情况下alpha==0.5效果可能会好一点, 正样本少的情况下用0.25
        Usage is same as nn.BCEWithLogits:
        >>> loss = criteria(logits, lbs)
        """
        super(FocalLossCSC, self).__init__()
    ...
    def forward(self, logits, labels):
        if self.activate_type == "softmax":
            idx = labels.long()
            one_hot_key = torch.zeros(idx.size(0), idx.size(1), dtype=torch.float32, device=idx.device)
            one_hot_key = one_hot_key.scatter_(1, idx, 1)
            logits = torch.softmax(logits, dim=-1)
            loss = -self.alpha * one_hot_key * torch.pow((1 - logits), self.gamma) * (logits + self.eps).log()
            loss = loss.sum(1)
        else:
...
class FocalLoss(nn.Module):
    ...
    """
    def __init__(self, activate_type="sigmoid", gamma=2.0, alpha=0.5, epsilon=1.e-9):
    ...
    def forward(self, input, target):
        ...
        if self.activate_type == "softmax":
            target = target.long()
            one_hot_key = torch.zeros(target.size(0), target.size(1), dtype=torch.float32, device=target.device)
            one_hot_key = one_hot_key.scatter_(1, target, 1)
            logits = torch.softmax(input, dim=-1)
            loss = -self.alpha * one_hot_key * torch.pow((1 - logits), self.gamma) * (logits + self.epsilon).log()
            loss = loss.sum(1)
        elif self.activate_type == "sigmoid":
...
class GridPointer(nn.Module):
    def __init__(self, head_nums, head_size, flag_RoPE=True):
        """GridPointer, 分类-网格(全局)指针模块, 因为更加稀疏了最好使用circle-loss, 否则可能不收敛.
...
class SpanFCLayer(nn.Module):
    def __init__(self, input_dim, output_dim, dropout_rate=0.1, flag_active=True,
                 flag_dropout=True, active_type="mish"):
        """SpanFCLayer
        ...
        args:
            ...
            flag_dropout: use dropout or not, 是否使用随机失活dropout, eg. True
            flag_active: use activation or not, 是否使用激活函数如tanh, eg. True
...
class CRF(nn.Module):
    ...
    code-from: https://github.com/kmkurn/pytorch-crf
    Args:
