...
from macro_correct.pytorch_sequencelabeling.slLayer import PriorMultiLabelSoftMarginLoss, MultiLabelCircleLoss, LabelSmoothingCrossEntropy
from macro_correct.pytorch_sequencelabeling.slLayer import CRF, SpanFCLayer, FCLayer, FocalLoss, DiceLoss, Swish, Mish, GridPointer
...
from macro_correct.pytorch_sequencelabeling.slConfig import PRETRAINED_MODEL_CLASSES, _SL_DATA_CONLL, _SL_DATA_SPAN


class Graph(BertPreTrainedModel):
    def __init__(self, graph_config, tokenizer, logger=None):
        """
        ...
        if self.graph_config.output_hidden_states:
            self.fc_span_start = SpanFCLayer(int(self.pretrained_config.hidden_size*len(self.graph_config.output_hidden_states)), self.graph_config.num_labels,
                                    flag_active=self.graph_config.flag_active, flag_dropout=self.graph_config.flag_dropout, active_type=self.graph_config.active_type)
            ...
            self.fc_span_end = SpanFCLayer(self.pretrained_config.hidden_size + dim_soft_label, self.graph_config.num_labels,
                                    flag_active=self.graph_config.flag_active, flag_dropout=self.graph_config.flag_dropout, active_type=self.graph_config.active_type)
            self.fc = FCLayer(self.pretrained_config.hidden_size, self.graph_config.num_labels, flag_active=self.graph_config.flag_active,
                              flag_dropout=self.graph_config.flag_dropout, active_type=self.graph_config.active_type)
            if self.graph_config.task_type.upper() in [_SL_MODEL_GRID]:
                self.fc = torch.nn.Linear(self.pretrained_config.hidden_size, self.graph_config.num_labels * self.graph_config.head_size * 2)
        # 网格(全局)指针网络, GPN
        self.layer_grid_pointer = GridPointer(head_nums=self.graph_config.num_labels, head_size=self.graph_config.head_size, flag_RoPE=True)
        # 条件随机场层, CRF
        ...
        self.loss_pmlsm = PriorMultiLabelSoftMarginLoss(prior=self.graph_config.prior, num_labels=self.graph_config.num_labels, task_type=self.graph_config.task_type)
        self.loss_focal = FocalLoss(activate_type="sigmoid")
        # self.loss_focal = FocalLoss(activate_type="softmax")
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, labels=None, labels_start=None, labels_end=None):
        ...
        if self.graph_config.task_type.upper() in [_SL_MODEL_SPAN]:
            ...
            if labels_start is not None and self.graph_config.flag_train:  # 训练时用labels校准, 推理时用logits
                if self.graph_config.flag_soft_label:
                ...
                if not self.graph_config.flag_soft_label:
        ...
        if labels is not None or labels_start is not None:  # loss
            ...
            if self.graph_config.task_type.upper() in [_SL_MODEL_CRF]:  # 条件随机场CRF损失
                ...
                # loss_1 = self.loss_focal(logits_start, labels_start)
                # loss_2 = self.loss_focal(logits_end, labels_end)
                # loss = loss_1 + loss_2
                ...
                loss = self.loss_ce(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            else:                                              # 其他情况: 二元交叉熵
                logits_softmax = self.softmax(logits)
                loss = self.loss_bce(logits_softmax.view(-1), labels.view(-1))
        # 预测阶段的输出等
        if self.graph_config.task_type.upper() in [_SL_MODEL_SOFTMAX]:
            logits = torch.argmax(logits_org, dim=-1)  # SL-SOFTMAX
        if self.graph_config.task_type.upper() in [_SL_MODEL_GRID]:
            logits = logits_org  # SL-GRID
        if self.graph_config.task_type.upper() in [_SL_MODEL_SPAN]:
            logits = torch.cat([logits_start_org, logits_end_org], 1)  # <32, 128+128, 7>
            logits_org = logits
        if self.graph_config.task_type.upper() in [_SL_MODEL_CRF]:
            logits = self.layer_crf.decode(logits_org, attention_mask).squeeze(0)  # CRF-Decode, 即标签tags
        return loss, logits, logits_org
