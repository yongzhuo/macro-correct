...
def mertics_report_sequence_labeling(y_true_dict, y_pred_dict, idx2label=None, rounded=4, flag_draw=True,
    ...
    Args:
        ...
        rounded    :  int,  最后保留小数位
        flag_draw   :  bool, 是否生成string格式的报告提供日志显示
    ...
    def draw_mertics_dict(metrics, rounded=4, flag_acc=False):
        """
        draw mertics, 绘画评估指标
        Args:
            metrics: dict, eg. {"游戏:132, "旅游":32}
            rounded: int, eg. 4
            flag_acc: bool, add acc or not, eg.True
        Returns:
            report: str
        """
        # 打印头部
        ...
        if not flag_acc and "acc" in headers: headers.remove("acc")
...
def padding_1d_2d_3d_4d_dim(data_arr, pad_len, pad_side="RIGHT", mode="constant", value=0):
    ...
    if pad_side.upper() == "RIGHT":
        ...
        padding_width = ((pad_len, 0),)
    if len(data_arr.shape) == 2:  # <32, 128>, y_true
        padding_width = ((0, 0),) + padding_width
    if len(data_arr.shape) == 3:  # <32, 128, 3>, y_pred: sl_softmax
        padding_width = ((0, 0),) + padding_width + ((0, 0),)
    if len(data_arr.shape) == 4:  # <32, 3, 128, 128>, y_pred: sl_grid
        padding_width = ((0, 0),) + ((0, 0),) + padding_width + padding_width
    try:
        data_arr = np.pad(data_arr, pad_width=padding_width, mode=mode, constant_values=value)
    except Exception as e:
        logging.info(str(e))
    return data_arr
...
def get_pos_from_span(logits_start, logits_end, i2l, flag_index=False):
    ...
    Args:
        ...
        flag_index: bool, True is from label, False is from logits
    Returns:
        pos: List, eg. [(人名, 0, 2)]
    """
    if not flag_index:
...
def transform_span_to_conll(y, label_id, l2i_conll, sl_ctype, flag_to_id=True):
    ...
    for i, yi in enumerate(y):
        ...
        if yi_pos_1 >= len(label_id):
            break
        if sl_ctype in ["BO"]:  # 单个字符
            for id in range(yi_pos[1] - yi_pos[0]+1):
                label_str[yi_pos_0 + id] = "B-" + yi_type
        elif sl_ctype in ["BIO", "OIB"]:
...
def softmax(x, axis=-1):
    ...
    Returns:
        s: np.array
    """
    s = np.exp(x) / np.sum(np.exp(x), axis=axis, keepdims=True)
    return s
...
if __name__ == '__main__':
    ...
    y = [{"type":"ORG", "ent":"沪", "pos":[2, 3]}]
    l2i = {"O":0, "B-ORG":1, "I-ORG":2}
    l2i_conll = {"O":0, "B-ORG":1, "I-ORG":2}
    sl_ctype = "BIO"
    label_id = [0, 0, 0, 0]
    label_id_conll = transform_span_to_conll(y, label_id, l2i_conll, sl_ctype)
    print(label_id_conll)
