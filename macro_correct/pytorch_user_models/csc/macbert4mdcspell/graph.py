# !/usr/bin/python
# -*- coding: utf-8 -*-
# @time    : 2021/1/18 21:34
# @author  : Mo
# @function: mdcspell-graph
# @paper   : [MDCSpell: A Multi-task Detector-Corrector Framework for Chinese Spelling Correction](https://aclanthology.org/2022.findings-acl.98/).


from __future__ import absolute_import, division, print_function
from copy import deepcopy
import argparse
import logging
import random
import math
import sys
import os

from torch.utils.data import DataLoader, RandomSampler, SequentialSampler, TensorDataset
from transformers import SchedulerType, get_scheduler
from transformers import BertConfig, BertModel
from transformers import BertPreTrainedModel
from transformers import BertForMaskedLM

from transformers import AutoTokenizer
import torch.nn as nn
from tqdm import tqdm
import numpy as np
import torch

from macro_correct.pytorch_textcorrection.tcLayer import MultiLabelCircleLoss, LabelSmoothingCrossEntropy
from macro_correct.pytorch_textcorrection.tcLayer import FocalLoss, DiceLoss


class Macbert4MDCSpell(BertPreTrainedModel):
    def __init__(self, config):
        self.config = BertConfig.from_pretrained(pretrained_model_name_or_path=config.pretrained_model_name_or_path)
        super().__init__(self.config)
        self.csc_config = config
        if self.csc_config.flag_train:
            self.config.update({"gradient_checkpointing": True})
            self.bert = BertForMaskedLM.from_pretrained(self.csc_config.pretrained_model_name_or_path)
        else:
            self.bert = BertForMaskedLM(self.config)
        # detect
        self.detect_macbert = nn.Linear(self.config.hidden_size, 1)
        # correct
        self.correct = nn.Linear(self.config.hidden_size, self.config.vocab_size)
        self.correct.weight.data = self.bert.bert.embeddings.word_embeddings.weight.data  # 初始化
        self.softmax = nn.Softmax(-1)
        # detect
        self.detect_layers = deepcopy(self.bert.bert.encoder.layer[:2])
        self.detect = nn.Linear(self.config.hidden_size, 1)
        self.emb = self.bert.bert.embeddings
        self.sigmoid = nn.Sigmoid()
        # 损失函数, loss
        self.loss_type = self.csc_config.loss_type if self.csc_config.loss_type else "BCE"
        self.loss_ce = torch.nn.CrossEntropyLoss(ignore_index=-100)
        self.loss_mlsm = torch.nn.MultiLabelSoftMarginLoss()  # like BCEWithLogitsLoss
        self.loss_bcelog = torch.nn.BCEWithLogitsLoss()
        self.loss_bce = torch.nn.BCELoss()
        self.loss_mse = torch.nn.MSELoss()
        self.loss_focal = FocalLoss(activation_type="sigmoid")
        self.loss_lsce = LabelSmoothingCrossEntropy()
        self.loss_circle = MultiLabelCircleLoss()
        self.loss_dice = DiceLoss()
        # self.post_init()

    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        out = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                        labels=labels, output_hidden_states=True, return_dict=True, )
        seq_output = out.hidden_states[-1]
        ### BertForModel需要的
        # seq_output = outputs.last_hidden_state  # seq_output
        # prediction_scores = self.cls(seq_output)  # hidden_states
        # det_macbert_probs = self.detect_macbert(outputs.hidden_states[-1])  # hidden_states[-1]
        ### det-layer
        position_ids = self.emb.position_ids[:, : input_ids.shape[1]]
        token_type_ids = torch.zeros(*input_ids.shape, dtype=torch.long, device=position_ids.device)
        det_output = self.emb.word_embeddings(input_ids) + \
                        self.emb.position_embeddings(position_ids) + \
                        self.emb.token_type_embeddings(token_type_ids)
        for layer in self.detect_layers:
            det_output = layer(det_output)[0]
        ### cor-layer
        cor_logits = self.correct(seq_output + det_output)
        if labels is not None:
            ### bert-mlm-loss, bert-det-loss
            # masked_lm_loss = self.loss_ce(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
            det_macbert_probs = self.detect_macbert(out.hidden_states[-1])  # hidden_states[-1]
            det_macbert_probs = det_macbert_probs.squeeze(-1) * attention_mask
            cor_macbert_probs = self.softmax(out.logits)
            cor_macbert_loss = out.loss
            ### cor-loss
            cor_loss = self.loss_ce(cor_logits.view(-1, self.config.vocab_size), labels.view(-1))
            ### det-layer
            det_probs = self.detect(det_output)
            det_labels = (input_ids != labels).float()
            det_probs = det_probs.squeeze(-1) * attention_mask

            if self.loss_type.upper() == "SOFT_MARGIN_LOSS":  # 边缘损失pytorch版, 划分距离
                det_macbert_loss = self.loss_mlsm(det_macbert_probs, det_labels)
                det_loss = self.loss_mlsm(det_probs, det_labels)
            elif self.loss_type.upper() == "FOCAL_LOSS":       # 聚焦损失(学习难样本, 2-0.25, 负样本多的情况)
                det_macbert_loss = self.loss_focal(det_macbert_probs, det_labels)
                det_loss = self.loss_focal(det_probs, det_labels)
            elif self.loss_type.upper() == "CIRCLE_LOSS":      # 圆形损失(均衡, 统一 triplet-loss 和 softmax-ce-loss)
                det_macbert_loss = self.loss_circle(det_macbert_probs, det_labels)
                det_loss = self.loss_circle(det_probs, det_labels)
            elif self.loss_type.upper() == "DICE_LOSS":        # 切块损失(图像)
                det_macbert_loss = self.loss_dice(det_macbert_probs, det_labels)
                det_loss = self.loss_dice(det_probs, det_labels.long())
            elif self.loss_type.upper() == "LABEL_SMOOTH":     # 交叉熵平滑
                det_macbert_loss = self.loss_lsce(det_macbert_probs, det_labels)
                det_loss = self.loss_lsce(det_probs, det_labels.long())
            elif self.loss_type.upper() == "BCE_LOGITS":       # 二元交叉熵平滑连续计算型pytorch版
                det_macbert_loss = self.loss_bcelog(det_macbert_probs, det_labels)
                det_loss = self.loss_bcelog(det_probs, det_labels)
            elif self.loss_type.upper() == "BCE_MULTI":        # 二元交叉熵的pytorch版, 多标签
                active_probs_macbert_sigmoid = self.sigmoid(det_probs)
                det_macbert_loss = self.loss_dice(active_probs_macbert_sigmoid, det_labels)
                active_probs_sigmoid = self.sigmoid(det_probs)
                det_loss = self.loss_bce(active_probs_sigmoid, det_labels)
            elif self.loss_type.upper() == "MSE":              # 均方误差
                det_macbert_loss = self.loss_mse(det_macbert_probs, det_labels)
                det_loss = self.loss_mse(det_probs, det_labels)
            else:  # 二元交叉熵的pytorch版, 多标签(BCE
                active_probs_macbert_sigmoid = self.sigmoid(det_macbert_probs)
                det_macbert_loss = self.loss_dice(active_probs_macbert_sigmoid, det_labels)
                active_probs_sigmoid = self.sigmoid(det_probs)
                det_loss = self.loss_bce(active_probs_sigmoid, det_labels)

            loss = (1-self.csc_config.loss_det_rate) * cor_loss + self.csc_config.loss_det_rate * det_loss
            loss_assistant = (1-self.csc_config.loss_det_rate) * cor_macbert_loss + self.csc_config.loss_det_rate * det_macbert_loss
            loss = 0.5 * loss + 0.5 * loss_assistant
            # print(cor_loss)
            # print(det_loss)
            # print(loss)
            # print("#"*128)
            if self.csc_config.flag_train:
                ### 输出mdcspell的
                cor_probs = self.softmax(cor_logits)
                pred_prob, pred_ids = torch.max(cor_probs, -1)
                pred_ids = pred_ids.masked_fill(attention_mask == 0, 0)
                outputs = (loss,
                           det_loss,
                           cor_loss,
                           det_macbert_loss,
                           cor_macbert_loss,
                           det_probs,
                           cor_probs,
                           pred_ids,
                           )
            else:
                ### 输出macbert的
                cor_probs = self.softmax(out.logits)
                pred_prob, pred_ids = torch.max(cor_probs, -1)
                pred_ids = pred_ids.masked_fill(attention_mask == 0, 0)
                outputs = (loss,
                           det_loss,
                           cor_loss,
                           det_macbert_loss,
                           cor_macbert_loss,
                           det_macbert_probs,
                           cor_macbert_probs,
                           pred_ids,
                           )
        else:
            cor_probs = self.softmax(out.logits)
            pred_prob, pred_ids = torch.max(cor_probs, -1)
            pred_ids = pred_ids.masked_fill(attention_mask == 0, 0)
            outputs = (cor_probs, pred_prob, pred_ids)
        return outputs

