...
from macro_correct.pytorch_user_models.csc.macbert4mdcspell.dataset import transfor_chinese_symbol_to_english, tradition_to_simple, string_q2b
...
from macro_correct.pytorch_user_models.csc.macbert4mdcspell.dataset import save_json, load_json, txt_write, txt_read
...
def eval_std_list(path_tet=None, path_model_dir=None, threshold=0.5, args=None):
    ...
    ### 加载模型
    model = MdcspellPredict(path_model_dir, path_model_dir)
    ...
    all_inputs = []
    datas_bs = []
    for d in tqdm(datas, desc="Evaluation-" + os.path.split(path_tet)[-1]):
        datas_bs.append(d)
...
def tet_test_dataset(path_model_dir=None, path_tet=None):
    ...
    path_model_dir = path_model_dir or "../../../output/text_correction/macbert4mdcspell_v1"
    # model = MdcspellPredict(path_model_dir, path_model_dir)
