...
if platform.system().lower() == "windows":
    csc_config = {
        ...
        "path_relm": "E:/DATA/bert-model/00_pytorch/CSC/relm-m0.3.bin",
        # relm权重, 论文提供的预训练wiki修改句对的权重, 来自[https://github.com/gingasan/lemon](https://github.com/gingasan/lemon)
        ...
        "task_name": "espell_law_std_of_relm",
        ...
        "train_batch_size": 4,  # 16*4=(noerror-mask--实际是error); # 32*4=0.8948(only-error-mask)
        "eval_batch_size": 4,
        "learning_rate": 3e-5,
        "max_seq_length": 128,  # 这里应该是双倍实际长度(实际的要除以2), constant=256; 256;384;512
        "max_grad_norm": 1.0,
    ...
    csc_config = {
        ...
        "path_relm": "relm-m0.3.bin",  # relm权重, 论文提供的预训练wiki修改句对的权重
        ...
        "eval_batch_size": 32,
        "learning_rate": 3e-5,
        "max_seq_length": 256,  # 这里应该是双倍实际长度(实际的要除以2), constant=256; 256;384;512
        "max_grad_norm": 1.0,
        "weight_decay": 5e-4,  # 5e-4
        "save_steps": 1000,
        "anchor": None,
