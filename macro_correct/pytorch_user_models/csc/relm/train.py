...
def train_csc():
    ...
    logging.basicConfig(format="%(asctime)s - %(levelname)s - %(name)s -   %(message)s",
                        datefmt="%m/%d/%Y %H:%M:%S",
                        level=logging.INFO,
                        filename=os.path.join(args.model_save_path, "train.relm.log"))
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features(train_examples, args.max_seq_length, tokenizer, args.prompt_length, anchor=anchor, logger=logger)
        ...
        # all_src_ref_ids = torch.tensor([f.src_ref_ids for f in train_features], dtype=torch.long)

        train_data = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_trg_ref_ids, all_block_flag)
        # train_data = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_trg_ref_ids, all_block_flag, all_src_ref_ids)
...
因为模型使用的是句对任务, 即NSP, 所以预训练该NSP句对任务比较好; 论文中的relm还是用上比较好;
序列为cls-sent1-sep-sent2-sep; wang271k没有提升; MFT的作用也很明显;
优点是比较适合少样本的few-shot类任务; 领域迁移也不错, 估计是句对任务有很多信息-copy;
缺点是NSP任务文本长度翻倍, 推理耗时增加1倍, 如何抉择还有待观察, 默认128*2=256;
"""
