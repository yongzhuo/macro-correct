...
def eval_std_list(path_tet=None, model_name_or_path=None, threshold=0.0):
    ...
    for line in datas:
        ...
        srcs.append(src[:128-2])
        tgts.append(tgt[:128-2])
    ...
    #     src_batch.append(s)
    #     if len(src_batch) == 32:
    #         res_i = model_macbert.predict(src_batch, **params)
    #         preds.extend([r.get("correct_text", "") for r in res_i])
    #         src_batch = []
    # if src_batch:
    #     res_i = model_macbert.predict(src_batch, **params)
    #     preds.extend([r.get("correct_text", "") for r in res_i])
    ...
    preds = []
    for idx, s in enumerate(tqdm(srcs, desc=file_name)):
        try:
            out = requests.post(url="http://127.0.0.1:8036/nlg/llm/eval_csc",
                          data={"query": s.strip()}
                          )
            if out.status_code == 200:
                out_dict = json.loads(out.text)
                out_data = out_dict.get("data", {})
                csc_2 = out_data.get("csc_2", "")
                # print("csc_1:", s)
                # print("csc_2:", csc_2)
                len_csc_2 = len(csc_2)
                len_s = len(s)
                len_mid = len_s - len_csc_2
                if len_mid <= 0:
                    csc_2 = csc_2[:len_s]
                else:
                    csc_2 += "".join(list(s)[-len_mid:])
            else:
                csc_2 = copy.deepcopy(s)
        except Exception as e:
            print(traceback.print_exc())
            csc_2 = copy.deepcopy(s)
        preds.append(csc_2)
    ...
    for s, p, t in zip(srcs, preds, tgts):
        line_dict = {"s": s, "t": t, "p": p}
        qwen_15_pred.append(line_dict)
    save_json(qwen_15_pred, path_tet + ".pred_qwen25_15.json")
    ...
    correction_precision, correction_recall, correction_f1 = char_mertic_det_cor(srcs, tgts, preds)
...
if __name__ == '__main__':
    ...
    ### 全量测试
    ### 模型权重
    path_model_dir = "../output/text_correction/shibing624_chinese-text-correction-1_5b/csc.config"  # own-correct
    threshold = 0.0
